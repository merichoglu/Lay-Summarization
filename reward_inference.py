...
# ------------------------------------------------------------------------------
def main():
    config_path = "reward_inference_config.json"
    start_time = time.time()
    logger.info("Starting reward model inference script")
    ...
    # -------------------- Model & Tokenizer --------------------
    logger.info("Loading reward-finetuned model and tokenizer...")
    ...
    # -------------------- Generate Summaries --------------------
    logger.info("Starting summary generation with reward model...")
    ...
    for i, input_text in enumerate(
        ...
        try:
            gen = gen_pipeline(input_text)
            full_generated = (
                gen[0]["generated_text"]
                if gen and isinstance(gen, list) and "generated_text" in gen[0]
                else ""
            )
            # Extract the summary from the generated text by splitting on "### Response:"
    ...
    try:
        ...
        logger.info("SARI result: %.4f", sari_result["sari"])
    except Exception as e:
        logger.error("Error computing SARI: %s", e)
